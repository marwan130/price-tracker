...
The backend is containerised with `docker/Dockerfile` and deployed automatically to Azure Container Apps on every push to `main` via `.github/workflows/azure-deploy.yml`.
